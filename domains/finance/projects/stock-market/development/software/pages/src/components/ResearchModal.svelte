...
End your response with a line: SUGGESTION: Direction=UP|DOWN, Weight=<number>`;
    ...
    try {
      ...
      if (match) {
        ...
        suggestedWeight = Math.min(100, Math.max(1, parseInt(match[2])));
        showSuggestion = true;
      }
    } catch (e: unknown) {
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="section">
        ...
        <div class="url-row">
          <input placeholder="https://..." bind:value={urlInput} on:keydown={e => e.key === 'Enter' && handleFetch()} />
          <button class="btn-fetch" on:click={handleFetch} disabled={loading}>
            {loading ? '…' : 'Fetch'}
          </button>
        </div>
        {#if fetchError}<div class="error-msg">{fetchError}</div>{/if}
        <textarea placeholder="Or paste text / notes here…" bind:value={pasteText} rows="3"></textarea>
        <button class="btn-add" on:click={addPaste} disabled={!pasteText.trim()}>Add to context</button>
          ...
          <div class="context-chips">
            {#each contextItems as item, i}
              <span class="context-chip">
                {item.label}
                <button on:click={() => contextItems = contextItems.filter((_, j) => j !== i)}>✕</button>
              </span>
            {/each}
          </div>
        {/if}
      ...
      <div class="section">
          ...
          <div class="chat-messages">
            {#each chatMessages as msg}
              <div class="chat-msg {msg.role}">
                <span class="chat-role">{msg.role === 'ai' ? '🤖' : '👤'}</span>
                <span class="chat-text">{msg.text}</span>
              </div>
            {/each}
          </div>
          <div class="chat-input-row">
            <input placeholder="Ask about this parameter…" bind:value={userInput}
        ...
        {#if showSuggestion && suggestedDir}
          <div class="suggestion-card">
            ...
            <button class="btn-apply" on:click={applySuggestion}>
              Apply
            </button>
          </div>
        {/if}
...
<style>
  ...
  .modal-subtitle { font-size: 0.75rem; color: var(--muted); margin-top: 0.2rem; max-width: 460px; }
  .close-btn {
    background: none; border: 1px solid var(--border); color: var(--muted);
    padding: 0.3rem 0.7rem; border-radius: 6px; font-size: 0.8rem; white-space: nowrap;
  }
  .modal-body { flex: 1; overflow-y: auto; padding: 1rem 1.25rem; display: flex; flex-direction: column; gap: 1rem; }
  .section { display: flex; flex-direction: column; gap: 0.5rem; }
  .section-label { font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--muted); }
  .url-row { display: flex; gap: 0.4rem; }
  .url-row input { flex: 1; }
  .btn-fetch { background: var(--accent); border: none; color: #fff; padding: 0.4rem 0.8rem; border-radius: 6px; font-size: 0.85rem; }
  .btn-add { background: none; border: 1px solid var(--border); color: var(--muted); padding: 0.3rem 0.7rem; border-radius: 6px; font-size: 0.8rem; align-self: flex-start; }
  .btn-add:disabled { opacity: 0.4; }
  textarea { resize: vertical; min-height: 60px; }
  .error-msg { font-size: 0.76rem; color: var(--danger); }
  .context-chips { display: flex; flex-wrap: wrap; gap: 0.35rem; }
  .context-chip {
    background: rgba(79,142,247,0.1); border: 1px solid rgba(79,142,247,0.2);
    color: var(--accent); font-size: 0.72rem; padding: 0.15rem 0.5rem;
    border-radius: 99px; display: flex; align-items: center; gap: 0.3rem;
  }
  .context-chip button { background: none; border: none; color: var(--muted); font-size: 0.65rem; padding: 0; cursor: pointer; }
  .llm-placeholder {
    background: rgba(79,142,247,0.06); border: 1px solid rgba(79,142,247,0.15);
    border-radius: 8px; padding: 1rem; display: flex; gap: 0.75rem;
  }
  ...
  .chat-messages { display: flex; flex-direction: column; gap: 0.5rem; max-height: 240px; overflow-y: auto; }
  ...
  .chat-input-row button { background: var(--accent); border: none; color: #fff; padding: 0.4rem 0.8rem; border-radius: 6px; }
  .suggestion-card {
    background: rgba(52,211,153,0.08); border: 1px solid rgba(52,211,153,0.2);
    border-radius: 8px; padding: 0.65rem 1rem; display: flex; align-items: center; gap: 0.75rem;
  }
  ...
  .btn-apply { margin-left: auto; background: var(--accent2); border: none; color: #000; padding: 0.3rem 0.8rem; border-radius: 6px; font-size: 0.82rem; font-weight: 600; }
</style>
